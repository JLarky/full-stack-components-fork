...
export function CustomerCombobox({ error }: { error?: string | null }) {
	const [customers0, customersSubmit] = createServerAction$(
		async ({ query }: { query: string }, { request }) => {
			// await requireUser(request)
			invariant(typeof query === 'string', 'query is required')
			return query.split('').reverse().join('')
		},
	)
	const [prevCustomers, setPrevCustomers] = createSignal(customers0.result)
	createEffect(() => {
		if (!customers0.pending) {
			setPrevCustomers(customers0.result)
		}
	})
	const customers = () => {
		return customers0.result || prevCustomers()
	}
	return (
		<div>
			<input name="customerId" type="hidden" value={customers() ?? ''} />
			<input
				class="w-full rounded border border-gray-300 p-2"
				oninput={e => {
					customersSubmit({ query: e.currentTarget.value })
				}}
			/>
			<div class="flex items-center p-2">
				<div class="mr-2">
					This string was reversed on server: {customers()}
				</div>
			</div>
		</div>
	)
...
// export function CustomerComboboxOld({ error }: { error?: string | null }) {
